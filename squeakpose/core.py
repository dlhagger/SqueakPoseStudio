...
def migrate_project_metadata(
    ...
    *,
    ...
    # Keep this import local so the low-level core module remains importable while
    # ``squeakpose.project`` is initializing. Project metadata owns layer semantics,
    # but legacy callers still reach this migration helper through ``squeakpose_core``.
    ...
    try:
        ...
        for record in reversed(records):
            ...
            try:
                ...
                if record_backup and os.path.lexists(str(record_backup)):
                    os.replace(str(record_backup), target_path)
