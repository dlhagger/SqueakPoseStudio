"""UI-neutral coordination around the persistent single-image prediction worker."""
...
class PredictionController(QObject):
    """Own request IDs, readiness queuing, and response correlation for prediction."""
    ...
    def __init__(
        ...
        *,
        ...
        arguments: tuple[str, ...] = ("-m", "predict_worker", "--server"),
    ...
    def submit_prediction(
        ...
        *,
        ...
        if self._busy or self._pending_request is not None:
            raise RuntimeError("prediction request already running")
        ...
        self._requested_image_path = image_path
        self._set_busy(True)
        self.status_changed.emit("Running prediction...")
    ...
    def warm_model(
        ...
        *,
        ...
        if self._busy or self._pending_request is not None:
            raise RuntimeError("prediction request already pending")
        request_id = self._next_request_id()
    ...
    def restart_model(
        ...
        *,
        ...
        """Stop the current generation and optionally warm a fresh one afterward."""

        self._restart_request = (
            {"layer_id": layer_id, "model_path": model_path, "device": device}
            if warm and model_path
            else None
        )
    ...
    def _send_or_queue(self, request: dict[str, Any]) -> None:
        ...
        if session.send_request(request):
            ...
            self.output_received.emit("Could not write prediction request to worker.")
    ...
    def _on_event(self, event: dict[str, Any]) -> None:
        ...
        if event_type == "loading":
            self.status_changed.emit("Loading prediction model...")
            return
        if event_type == "loaded":
            self.status_changed.emit("Prediction model ready.")
            return
        ...
        if decision.action == "background_error":
            self.status_changed.emit(f"Prediction model error: {decision.error_message}")
    ...
    def _on_terminal(self, result: PersistentWorkerResult) -> None:
        ...
        if self._busy:
            if cancel_requested:
                decision = PredictionEventDecision(
                    "cancel", request_id=self._current_request_id, matched=True
                )
            elif not expected_stop:
                message = result.error_message or result.stderr or "Prediction worker stopped."
                decision = PredictionEventDecision(
                    "error",
