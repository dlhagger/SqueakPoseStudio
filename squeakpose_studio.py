...
def _preferred_distillation_export(run_dir: str) -> str:
    ...
    for candidate in preferred:
        if os.path.isfile(candidate):
            return candidate
    try:
        for name in sorted(os.listdir(exported_dir)):
...
def _discover_distillation_exports(search_roots: list[tuple[str, str]]) -> list[tuple[str, str]]:
    ...
    for source_label, root in search_roots:
        ...
        for run_dir in run_dirs:
            ...
            checkpoint_path = _preferred_distillation_export(normalized_run_dir)
...
def _distillation_sample_count(total_frames: int, stride: int, max_frames: int = 0) -> int:
    """Return the number of frames selected by a regular sampling interval."""
...
class DistillationDialog(QDialog):
    ...
    def __init__(self, parent):
        ...
        self.app_base_dir = os.path.abspath(
            getattr(parent, "app_base_dir", os.path.dirname(os.path.abspath(__file__)))
        )
